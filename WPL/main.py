...
def index():
    if db.session.execute('select count() from user').scalar() == 0:
        user = User(email='adminwpl2023@gmail.com', \
                    password=generate_password_hash('Admin@wpl2023', method='sha256'), \
                    name='AdminWPL2023')
        db.session.add(user)
        db.session.commit()
    ...
    if db.session.execute('select count() from squad').scalar() == 0:
        ...
        for i in df[1:]:
            pl = Squad(Player_ID=i[0], Name=i[1], Team=i[2], Captain=i[3], Keeper=i[4], Overseas=i[5],\
                       ...
                       DOB=(datetime.strptime(i[10],'%Y-%m-%d')).date())
            db.session.add(pl)
            db.session.commit()
